...
const TABS = {
  ...
  igViews:       717857799,
  fbViews:       1738697224,
  fbFollowers:   612313852,
  igFollowers:   128001537,
};
...
async function processReservations() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (visitRaw.match(/\d+\/\d+\/\d+/)) {
      ...
      const vyFull = vy.length === 2 ? '20' + vy : vy;
      visitKey = `${vyFull}-${vm.padStart(2,'0')}-${vd.padStart(2,'0')}`;
    }
    const email = (col[14]||'').trim().toLowerCase();
    const phone = (col[13]||'').trim().replace(/\D/g,'');
    const name  = (col[11]||'').trim().toLowerCase();
...
export default async function handler(req, res) {
  ...
  const [ga, facebook, googleAds, googlePrivate, reservations, perfectVenue, email, igVisits, webSources, igViewsData, fbViewsData, fbFollowersData, igFollowersData] = await Promise.all([
    ...
    tryFetch(processIgViews), tryFetch(processFbViews), tryFetch(processFbFollowers), tryFetch(processIgFollowers),
  ]);
  ...
  res.status(200).json({
    ...
    igViewsData, fbViewsData, fbFollowersData, igFollowersData,
