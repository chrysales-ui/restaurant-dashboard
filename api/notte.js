const SHEET_ID = '19bp0Gxn2v8yYGjAoFYYGgE6XIr3CVZVynY73eSghue0';
const TABS = {
  ga:            255801652,
  facebook:      1312145769,
  igVisits:      267788399,
  ...
  emails:        1631206564,
  igViews:       1751454445,
  igFollowers:   1833853153,
  fbViews:       645244231,
  fbFollowers:   330113669,
};
...
async function tryFetch(fn) {
  try { return await fn(); } catch (e) { console.error(e.message); return null; }
}
...
async function processReservations() {
  ...
  // New OpenTable export: col[0]=Visit Date, col[1]=Visit Time, col[2]=Created Date (PRIMARY)
  // col[11]=Guest Name, col[13]=Phone, col[14]=Email, col[15]=Marketing Opt-In, col[19]=Party Size, col[20]=Status
  ...
  for (let i = 1; i < lines.length; i++) {
    const col = parseCSVLine(lines[i]);
    const dk = parseDate(col[2]);
    if (!dk || dk < '2020-01-01') continue;
  ...
  for (let i = 1; i < lines.length; i++) {
    const col = parseCSVLine(lines[i]);
    const dateKey = parseDate(col[2]);
    if (!dateKey || dateKey < '2020-01-01') continue;
    ...
    const visitKey = parseDate(col[0]) || '';
    const email = (col[14]||'').trim().toLowerCase();
    const phone = (col[13]||'').trim().replace(/\D/g,'');
    const name  = (col[11]||'').trim().toLowerCase();
    const guestId = email || phone || name || null;
...
async function processEmail() {
  ...
  for (let i = 1; i < sentLines.length; i++) {
    ...
    const mo = (col[3]||'').trim().match(/(\d+)\/(\d+)\/(\d+)/);
    if (!mo) continue;
    const y = mo[3].length === 2 ? '20' + mo[3] : mo[3];
    const key = `${y}-${mo[1].padStart(2,'0')}`;
    const date = `${y}-${mo[1].padStart(2,'0')}-${mo[2].padStart(2,'0')}`;
    const status = (col[1]||'').trim();
    if (!monthly[key]) monthly[key] = { month:key, total:0, delivered:0, opened:0, clicked:0, unsubscribed:0, replied:0 };
    if (!dailyMap[date]) dailyMap[date] = { date, total:0, opened:0, clicked:0 };
    monthly[key].total++; dailyMap[date].total++;
    if (status.includes('Delivered')) monthly[key].delivered++;
    if (status.includes('Opened')) { monthly[key].opened++; dailyMap[date].opened++; }
    if (status.includes('Clicked')) { monthly[key].clicked++; dailyMap[date].clicked++; }
    if (status.includes('Unsubscribed')) monthly[key].unsubscribed++;
    if (status.includes('Replied')) monthly[key].replied++;
  }
  const arr = Object.values(monthly).sort((a,b)=>a.month.localeCompare(b.month));
  arr.forEach(m => {
...
async function processSocialDaily(gid, field, startRow) {
  const lines = await fetchCSV(gid);
  const daily = [];
  for (let i = startRow; i < lines.length; i++) {
    const col = parseCSVLine(lines[i]);
    if (!col[0]) continue;
    let date;
    if (col[0].match(/^\d{4}-\d{2}-\d{2}/)) date = col[0].trim().slice(0, 10);
    else if (col[0].match(/\d+\/\d+\/\d+/)) {
      const [m, d, y] = col[0].trim().split('/');
      date = `${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;
    } else continue;
    daily.push({ date, [field]: cleanNum(col[1]) });
  }
  return daily.sort((a, b) => a.date.localeCompare(b.date));
}
function processIgViews()     { return processSocialDaily(TABS.igViews,     'views', 2); }
function processFbViews()     { return processSocialDaily(TABS.fbViews,     'views', 1); }
function processFbFollowers() { return processSocialDaily(TABS.fbFollowers, 'netFollowers', 1); }
function processIgFollowers() { return processSocialDaily(TABS.igFollowers, 'netFollowers', 1); }

export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 'no-store');
  try {
    const [ga, facebook, googleAds, googlePrivate, reservations, perfectVenue, email, igVisits, webSources, igViewsData, fbViewsData, igFollowersData, fbFollowersData] = await Promise.all([
      tryFetch(processGA), tryFetch(processFacebook), tryFetch(processGoogleAds), tryFetch(processGooglePrivate),
      tryFetch(processReservations), tryFetch(processPerfectVenue), tryFetch(processEmail), tryFetch(processIgVisits), tryFetch(processWebSources),
      tryFetch(processIgViews), tryFetch(processFbViews), tryFetch(processIgFollowers), tryFetch(processFbFollowers),
    ]);
    const summary = ga ? { daily: ga.daily, monthly: ga.monthly } : null;
    res.status(200).json({
      summary, ga, facebook, googleAds, googlePrivate,
      reservations, perfectVenue, email, igVisits, webSources,
      igViewsData, fbViewsData, igFollowersData, fbFollowersData,
